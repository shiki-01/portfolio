<script lang="ts">
	...
	const sections: Array<{ label: string; items: SitemapItem[] }> = [
		{
			label: 'Pages',
			items: [
				{
					title: 'Home',
					href: '/',
					description: 'トップページ。自己紹介・スキル・制作実績のハイライト。'
				},
		...
		{
			label: 'Links',
			items: icons.map(({ title, link, icon }) => ({
				title,
				href: link,
				description: link,
				icon,
...
<Page height={100}>
	<div
		class="w:100% h:100% pt:64px px:clamp(20px,6vw,100px) flex flex:column ai:start gap:80px pb:80px"
	>
		<Title>sitemap</Title>
		<div class="w:100% flex flex:column gap:60px">
			{#each sections as section}
				<section class="flex flex:column gap:20px">
					<h2
						class="text:2em font:thin uppercase opacity:.7 bb:1px|solid|rgba(255,255,255,0.3) pb:12px"
					>
						{section.label}
					</h2>
